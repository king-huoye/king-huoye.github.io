...
**修正后的位置编码公式，使用 `\begin{cases}` 环境：**
$$
\begin{cases}
\text{Positional Encoding}(i, 2k)=\sin \left(i / 10000^{2k / d_{\text{model}}}\right) \\
\text{Positional Encoding}(i, 2k+1)=\cos \left(i / 10000^{2k / d_{\text{model}}}\right)
\end{cases}
$$
...
$$
\begin{cases}
\text{Positional Encoding}(i, 2k) = \sin \left( \frac{i}{10000^{2k / d_{\text{model}}}} \right) \\
\text{Positional Encoding}(i, 2k+1) = \cos \left( \frac{i}{10000^{2k / d_{\text{model}}}} \right)
\end{cases}
$$
...
**通过这种方式，每个位置会生成一个与 $d_{\text{model}}$ 维度相同的向量**，具有以下特点：
- 不同位置的编码是不同的。
- 编码在嵌入维度上的变化具有一定的周期性。
...
$A_{masked} = A + M$
其中，$M$ 对未来位置进行遮挡，设置为一个很小的值（如 -∞）。最终的得分会通过 **softmax** 函数来归一化，保证只有当前和之前的时间步得到有效的注意力权重。
...
- **测试时**，Dropout 的处理方式与训练时有所不同。在推理阶段（即测试时），**不再进行随机丢弃神经元**，而是使用所有神经元。然而，为了保持训练和推理阶段的输出一致，**需要将神经元的输出按照 Dropout 比例缩放**。即，每个神经元的激活值会乘以一个系数（通常是 $1-\text{dropout\_rate}$）。
...
- Transformer 的**传统自注意力机制会对未来位置进行遮蔽**，通常用于**生成任务**，目的**是避免模型看到未来的单词**。
